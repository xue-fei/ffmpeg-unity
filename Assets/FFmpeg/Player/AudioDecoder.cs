...
public unsafe class AudioDecoder : IMedia
{
    //媒体格式容器
    ...
    {
        var da = sourceFrame->data;

        var tempBufferPtr = bufferPtr;

        var v = sourceFrame->extended_data;
        var ptr = (IntPtr)v;
        var d = **v;
        var by = new byte[sourceFrame->nb_samples];
        Marshal.Copy(ptr, by, 0, by.Length);
        //IntPtr ptr = v;

        //Marshal.Copy()
        //重采样音频
        var outputSamplesPerChannel = ffmpeg.swr_convert(convert, &tempBufferPtr, sourceFrame->nb_samples, sourceFrame->extended_data, sourceFrame->nb_samples);
        //获取重采样后的音频数据大小
        ...
        //    {
        //        if (i > PreviousBytes.Length - 1)
        //            break;
        //        var b = (byte)(bytes[i] * 0.5);
        //        var a = (byte)(PreviousBytes[i] * 0.5);
        //        bytes[i] = (byte)(a + b);
        //    }
        //}

        PreviousBytes = tempBytes;
        return bytes;
    }
    public void Play()
    {
